  ...
  share: async function (e) {
    ...
    if (this.shareFeatureDetection() === true) {
      const shareData = {
        ...
        text:
          "COVID Tenders is providing an open, public database of COVID-19 innovations. Suppliers of tech solutions can directly post their details online, and be openly searched and browsed by anyone who has a demand, in a peer-to-peer fashion.",
      ...
      alert(
        "Share this text which has been copied to your clip board.\n\r " +
          stringToCopy
      );
    }
  },
...
COVID.Solutions = {
  ...
  create_links: function (node) {
    ...
    for (var i = 0; i < COVID.Solutions.solution_order.length; i++) {
      ...
      if (data != undefined) {
        ...
        li.setAttribute("class", "nav-item-test");
        li.setAttribute("id", key);

        var atag = document.createElement("a");
        atag.setAttribute("class", "nav-link");
        var href = "javascript:COVID.Solutions.load_category_index({0});".format(
          i
        );
  ...
  load_category_index: function (index) {
    ...
    if (data != undefined) {
      ...
      var src = "https://airtable.com/embed/{0}?backgroundColor=blue&layout=card&viewControls=on".format(
        data.key
      );
      iframe.setAttribute("src", src);

      var nodes = document
        .getElementById("sol_links")
